...
<div class="cart_wrapper clearfix{if $PS_CATALOG_MODE} header_user_catalog{/if}">
{/if}
	<div class="shopping_cart">
		<a href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" id="fruit_cart" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
			<b class="header_hide">{l s='Shopping cart' mod='blockcart'}</b><i class="shopping_cart_icon"></i>
			...
			<span class="ajax_cart_no_product{if $cart_qties > 0} unvisible{/if}">{l s='0' mod='blockcart'}</span> <!-- empty -->
					...
					<div class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
									...
									<dt data-id="cart_block_product_{$product.id_product|intval}_{if $product.id_product_attribute}{$product.id_product_attribute|intval}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery|intval}{else}0{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
										...
										<div class="cart-info">
											<div class="product-name">
												...
												<span class="quantity-formated">&nbsp;x&nbsp;
												<span class="quantity">{$product.cart_quantity}</span></span>
											...
											{/if}
											<span class="price">{if !isset($product.is_gift) || !$product.is_gift}{if $priceDisplay == $smarty.const.PS_TAX_EXC}{displayWtPrice p="`$product.total`"}{else}{displayWtPrice p="`$product.total_wt`"}{/if}<div class="hookDisplayProductPriceBlock-price">{hook h="displayProductPriceBlock" product=$product type="price" from="blockcart"}</div>
												{else}
						...
						<div class="cart-prices">
							...
							<div class="cart-prices-line last-line">
								...
								<span>{l s='Total price' mod='blockcart'}</span>
							</div>
						...
						<p class="cart-buttons">
							<a id="button_order_cart" class="cart_button" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">
								<span>
		...
		<div class="clearfix">
			...
			<div class="layer_cart_cart col-xs-12 col-md-6">
				...
				<div class="button-container">
					<span class="continue btn btn-default button button-medium" title="{l s='Continue shopping' mod='blockcart'}">
						<span>
